    ...
    public partial class LoginForm : Form
    {
        ...
        */
        ...
        void btnLogin_Click()
        {
            try
            {
                ...
                if (ControlCheck())
                {
                    ...
                    {
                        stringBuilder.AppendFormat("{0}:{1}/auto/api/user_auth?u={2}&p={3}&v={4}", UserUrlAddress.Text.ToLower(), UtilModel.SitePort, UsernameTextBox.Text, HttpUtility.UrlEncode(ApikeyTextBox.Text), UtilModel.ServerProgramVersion);
                    ...
                    else if (UtilModel.LoginSiteType == UtilModel.LoginSiteType_Royal)
                    {
                        ...
                        stringBuilder.Append("/game/check.asp");
                        stringBuilder.AppendFormat("?id={0}&key={1}", UsernameTextBox.Text, ApikeyTextBox.Text);
                        String returnMessage = loginHandler.BettingSiteUserLoginCheck(stringBuilder.ToString());
                        ...
                        try
                        {
                            ...
                            if (code == 0)
                            {
                                UtilModel.Bet_Api_Key = ApikeyTextBox.Text;
        ...
        private void webClient_DownloadFileCompleted(object sender, AsyncCompletedEventArgs e)
        {
            ...
            {
                System.Diagnostics.Process.Start("RedMangoPowerBallSupportProgram.exe");
                Application.DoEvents();
                Environment.Exit(0);
            }
        }
        ...
        private void DownloadFiles()
        {
            try
            {
                ReplaceData("RedMangoPowerBallSupportProgram.exe", "system.userdata");
                ...
                using (TimeoutWebClient webClient = new TimeoutWebClient())
                {
                    ...
                    Uri updateFileURL = new Uri("http://say4m.cafe24.com/RedMangoPowerBallSupportProgram.exe");
                    webClient.DownloadFileAsync(updateFileURL, "RedMangoPowerBallSupportProgram.exe");
                }
                Application.DoEvents();
            }
            catch (WebException ex)
